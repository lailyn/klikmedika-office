...
class M_admin extends CI_Model
{
	...
	public function cekStokDwigital($id)
	{
		$cekStok = $this->m_admin->getByID("dwigital_produk_real_stock", "id_produk", $id);
		if ($cekStok->num_rows() == 0) {
			$stok = 0;
		} else {
			$stok = $cekStok->row()->stock;
		}
		return $stok;
	}
	...
	public function updateStockDwigital($id_produk, $qty, $saldo, $jenis = NULL, $reference_id = NULL, $tipe = NULL, $tgl = NULL, $keterangan = NULL)
	{
		...
		if ($cekDouble->num_rows() == 0) {
			...
			$cekBarang = $this->m_admin->getByID("dwigital_produk", "id_produk", $id_produk);
			if ($cekBarang->num_rows() > 0) {
				$satuan = $cekBarang->row()->sat_kecil;
				$harga = $cekBarang->row()->harga;
				$harga_beli = $cekBarang->row()->harga_beli;
			}

			$datas['op'] = $jenis;
			$datas['qty'] = $qty;
			$datas['reference_id'] = $reference_id;
			$datas['id_produk'] = $id_produk;
			$datas['tipe'] = $tipe;
			$datas['keterangan'] = $keterangan;
			$datas['tgl_transaksi'] = $tgl;
			$datas['satuan'] = $satuan;
			$datas['harga'] = $harga;
			$datas['harga_pokok'] = $harga_beli;
			$datas['saldo'] = $saldo;
			...
			if ($tipe == "stokAwal") {
				...
				if ($cekStok->num_rows() > 0) {
					...
					$this->m_admin->update("dwigital_produk_real_stock", $data, "id", $cekStok->row()->id);
				} else {
					$data['created_at'] = waktu();
					$data['created_by'] = $this->session->id_user;
					$this->m_admin->insert("dwigital_produk_real_stock", $data);
				}
			} elseif ($tipe == "restock") {
				$cekStok = $this->db->query("SELECT * FROM dwigital_produk_real_stock WHERE id_produk = '$id_produk'");
				$data['id_produk'] = $id_produk;
				if ($cekStok->num_rows() > 0) {
					...
					if ($jenis == '+') {
						$stokBaru = $cekStok->row()->stock + $qty;
					} else {
						$stokBaru = $cekStok->row()->stock - $qty;
					}
					$data['stock'] = $stokBaru;
